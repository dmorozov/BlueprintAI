...
- **Leading hypothesis as of 2026-09-14 is now H0 (app-side): the AR engine is never instantiated** — see ticket 00. Work it before the upstream hypotheses; tickets 01/02 are still worth running (they cost minutes and settle the platform question either way), but their interpretation changes once a navigator is actually mounted.
...
- [The app never mounts ViroARSceneNavigator: no AR engine is ever created](issues/00-mount-viro-scene-navigator.md) *(open — the finding is settled, the fix and device test are not)*: `src/` renders `<ViroARScene>` bare and never has (`grep`/`git log -S` both empty); only `VRTARSceneNavigator` constructs `ViroViewARCore`, the object that creates/resumes the ARCore session and draws the camera. This reproduces the Dead session exactly and explains both log lines in the original write-up without any working session. **Displaces H1–H4 as the leading hypothesis.**
