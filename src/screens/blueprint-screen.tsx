...
export function BlueprintScreen() {
  ...
  const isArMeasured = useMemo(
    () => (roomId !== '' ? getPlanSource(roomId) === 'ar-tap' : false),
    [roomId],
  );

  // True when the plan came from the self-hosted photo assist (MoGe suggestions the
  // user confirmed). Also never regenerated by the mock provider, and always labeled
  // "auto-detected — verify" so it is not mistaken for a real measurement.
  const isPhotoAssist = useMemo(
    () => (roomId !== '' ? getPlanSource(roomId) === 'photo-assist' : false),
    [roomId],
  );
            ...
            <View style={styles.summary}>
              <ThemedText variant="small" themeColor="textSecondary">
                {isArMeasured
                  ...
                  : isPhotoAssist
                    ? 'Auto-detected from photos by your self-hosted assist server — verify against the real room.'
                    : provider.id === 'mock'
